...
class StateMachine:
    ...
    def get_math_prompt(self, solution: Dict[str, Any], session_history: list,model) -> str:
        ...
        if self.state == InterviewState.SESSION_START:
            ...
            return prompt
        return ""
    def get_stem_prompt(self, solution: Dict[str, Any], session_history: list, action: str = None,model = 'gpt-4o') -> str:
        if self.state == InterviewState.SESSION_START:
            inital_question = f'''{solution['initial_question']}'''.replace('"', "'")
            if str(self.action) == "paraphrasing":
            ...
            if self.attempts == 0:
                ...
                # else:
                evaluation = grader_stem(solution,session_history, attempt=self.attempts,assessment = self.TF[-1],recall=False,model = model)
                self.cost_grader.append(evaluation.usage_metadata)
            evaluation_json = extract_json(evaluation.content)
            if type(evaluation_json) == str:
                evaluation_json = extract_json(evaluation_json)
            ...
            grade_recall = grader_stem(solution,session_history, attempt=self.attempts,assessment = self.TF[-1],recall=True,model = model)
