...
from src.base_grader import grader_math,uncer_math,grader_stem, acc_counter_stem
...
class StateMachine:
    ...
    def get_prompt(self, solution: Dict[str, Any], session_history: list) -> str:
        if self.interview_type == InterviewType.MATH:
            ...
            return self.get_stem_prompt(solution, session_history)
    ...
    def get_math_prompt(self, solution: Dict[str, Any], session_history: list) -> str:
        ...
        if self.state == InterviewState.SESSION_START:
            ...
            evaluation = uncer_math(solution,session_history[-1])
            ...
            evaluation = grader_math(solution,session_history[-1])
            ...
            return prompt
        return ""
    def get_stem_prompt(self, solution: Dict[str, Any], session_history: list, action: str = None) -> str:
        if self.state == InterviewState.SESSION_START:
            inital_question = f'''{solution['initial_question']}'''.replace('"', "'")
            if str(self.action) == "paraphrasing":
            ...
            if self.attempts == 0:
                evaluation = grader_stem(solution,session_history, attempt=self.attempts,recall=False)
                self.cost_grader.append(evaluation.usage_metadata)
            else:
                # if self.TF_answer:
                #     evaluation = grader_stem(solution,session_history, attempt=self.attempts,model_output = self.TF_answer,recall=True)
                # else:
                evaluation = grader_stem(solution,session_history, attempt=self.attempts,assessment = self.TF[-1],recall=False)
                self.cost_grader.append(evaluation.usage_metadata)
            evaluation_json = extract_json(evaluation.content)
            if type(evaluation_json) == str:
                evaluation_json = extract_json(evaluation_json)
            ...
            grade_recall = grader_stem(solution,session_history, attempt=self.attempts,assessment = self.TF[-1],recall=True)
            ...
            if self.recall == 1:
                ...
                prompt = STEM_EVALUATOR_STATE_FOLLOWUP_QUESTION_PROMPT_TEMPLATE.format(question = solution['initial_question'], solution = grade_recall)
                return prompt     
        elif self.state == InterviewState.FOLLOW_UP_QUESTION_SOLVING:
         ...
         #   grade_recall = grader_stem(solution,session_history, attempt=1,assessment = self.TF[-1],recall=True)
            ...
            if self.attempts == 0:
                ...
                prompt = STEM_EVALUATOR_STATE_FOLLOWUP_QUESTION_1_PROMPT_TEMPLATE.format(question = solution['initial_question'], solution = solution['solution'],history = session_history[-1], followup =self.folloup_question)
                return prompt   
            else:
                prompt = STEM_EVALUATOR_STATE_FOLLOWUP_QUESTION_2_PROMPT_TEMPLATE.format(question = solution['initial_question'], solution = solution['solution'],correction = session_history[-1], feedback = session_history[-2],history = self.follow_up_types[-1],followup =self.folloup_question)
    ...
    def extract_math_message(self, solution: Dict[str, Any],message: str) -> Tuple[str, str]:
        if self.state == InterviewState.CLARIFICATION_NEEDED:
            ...
            try:
                ...
                return f"You need the fact that {fact}, which leads to the question {solution['initial_question']}", "complete"
        elif self.state == InterviewState.QUESTION_SOLVING:
