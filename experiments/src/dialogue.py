...
from src.base_state import StateMachine, InterviewState
...
class Moderator:
    ...
    def receive_and_send(self, speaker: str, message: str) -> tuple[str, str]:
        if speaker == self.agents_name[0]:  # evaluatee
            ...
            self.session_history.append(f"{speaker}: {message}")
            self.declare_state()
            if self.state_machine.state.name not in ['SOLVE_SUCCESS','SOLVE_FAIL','QUESTION_COMPLETE', 'EVALUATION_COMPLETE','MOVING_TO_NEXT_QUESTION']:
    ...
    def declare_state(self):
        ...
        self.state = self.state_machine.transition(self.action, self.state_threshold,self.state_threshold_followup,self.followup_flag)
    ...
    def _summarize(self, session_history):
        ...
        prompt = SESSION_SUMMARIZE_PROMPT.format(system_name=self.agents_name[0], user_name=self.agents_name[1], session_history=session_history)
...
class DialogueSimulator:
    ...
    def step(self) -> tuple[str, str]:
        ...
        if self.moderator.state_machine.state.name not in ['QUESTION_COMPLETE', 'EVALUATION_COMPLETE','MOVING_TO_NEXT_QUESTION']:
    ...
    def save_attributes_to_file(self,output_path):
        ...
        head = ['idx','solution','session_history','state_flow','correctness_main','correctness_sub_main','correctness_main_detail','correctness_followup','error_type','feedback_types','followup_types','cost_grader','cost_interviewer','cost_interviewee','duration']
        value = [self.moderator.current_query_index,self.moderator.current_query,self.moderator.session_history,self.moderator.session_state_flow,self.moderator.state_machine.success,self.moderator.state_machine.recall,self.moderator.state_machine.TF,self.moderator.state_machine.success_followup,self.moderator.state_machine.error_types,self.moderator.state_machine.feedback_types,self.moderator.state_machine.follow_up_types,self.moderator.state_machine.cost_grader,self.agents[self.moderator.agents_name[1]].cost,self.agents[self.moderator.agents_name[0]].cost,self.duration]
