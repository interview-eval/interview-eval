import re
import json
from math_reasoning.prompt import MATH_GRADER_PROMPT_TEMPLATE,MATH_EVALUATOR_STATE_UNC_0_PROMPT_TEMPLATE
from stem.prompt import STEM_GRADER_0_PROMPT_TEMPLATE, STEM_GRADER_1_PROMPT_TEMPLATE,STEM_GRADER_1_RECALL_PROMPT_TEMPLATE, STEM_GRADER_0_PRECISION_PROMPT_TEMPLATE,STEM_GRADER_0_RECALL_PROMPT_TEMPLATE,STEM_GRADER_1_RECALL_PROMPT_TEMPLATE,STEM_GRADER_1_PRECISION_PROMPT_TEMPLATE
from src.models import ChatModel
from src.utils import extract_json
from json_repair import repair_json

def grader_math(query,model_output,model = 'gpt-4o'):
    prompt = MATH_GRADER_PROMPT_TEMPLATE.format(question = query['revised_question'],
    answer = query['answer'] , solution = query['solution'],history = model_output )
    if type(model)==str:
...
def grader_stem(query,history,model = 'gpt-4o', attempt = 0,assessment=None,recall=False):
    ...
    if recall:
        if attempt == 0:
            prompt = STEM_GRADER_0_RECALL_PROMPT_TEMPLATE.format(question = query['revised_question'],
            solution = query['solution_atom'],history = assessment )

            output = grader.invoke(prompt)
            return output
        else:
            prompt = STEM_GRADER_1_RECALL_PROMPT_TEMPLATE.format(question = query['revised_question'],
            feedback = history[-2],correction = history[-1],
            solution =assessment)
        ...
        #     return output.content
            ...
            prompt = STEM_GRADER_1_PRECISION_PROMPT_TEMPLATE.format(question = query['revised_question'],
            feedback = history[-2],correction = history[-1],
            solution = query['solution'],history = assessment)
    ...
    if recall:
        if type(query) == dict:
            try:
                ...
                correct_facts += sum(0.5 for fact in query["reference_facts"].values() if fact["label"] == "partially supported")
    ...
    if followup:
        if type(query) == dict:
            try:
                ...
                correct_facts = sum(1 for fact in query["follow-up question"].values() if fact["correctness"] == "correct")
                correct_facts += sum(0.5 for fact in query["follow-up question"].values() if fact["correctness"] == "partially correct")
                # base_rubric = {'high':1,'middle':0,'low':-1}
                # redun_rubric = {'low':1,'middle':0,'high':-1}
        ...
        if type(query) == dict:
            try:
                ...
                try:
                    supported_facts = sum(1 for fact in query["model_atomic_facts"] if fact["reference solution coverage"] == "supported")
                except:
                    try:
                        supported_facts = sum(1 for fact in query["model_atomic_facts"] if fact["reference_solution_coverage"] == "supported")
                    except:
                        supported_facts = 0
                base_rubric = {'high':1,'middle':0,'low':-1}
                redun_rubric = {'low':1,'middle':0,'high':-1}
...
def uncer_math(solution,model_output,model = 'gpt-4o'):
    prompt = MATH_EVALUATOR_STATE_UNC_0_PROMPT_TEMPLATE.format(initial_question=f'''{solution['initial_question']}''',
