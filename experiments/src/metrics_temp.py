...
from utils import write_csv_row,extract_json
...
def calculate_accuracies(df,task):
    ...
    for idx, row in df.iterrows():
        if task == 'math':
            ...
            question_solving_count = state_flow.count("QUESTION_SOLVING")
            followup = eval(row['correctness_followup'])
    ...
    try:
        ...
        follwoup_Acc = followup_sum/followup_tot
    except:
        import pdb;pdb.set_trace()
        return [0],[0],[0]
    return [accuracy_A,quality_A,num_fact_A], [accuracy_B,quality_B,num_fact_B], [accuracy_C,quality_C,num_fact_C], follwoup_Acc
...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        try:
            accuracy_A, accuracy_B, accuracy_C,follwoup_Acc = calculate_accuracies(df,task)
        except:
            import pdb;pdb.set_trace()
            continue
        ...
        if task == 'math':
            ...
            write_csv_row([model]+[accuracy_A]+[accuracy_B]+[accuracy_C]+[follwoup_Acc],output_path)
