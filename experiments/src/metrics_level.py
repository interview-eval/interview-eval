...
def calculate_accuracies(df,task):
    ...
    return accuracy_A, accuracy_B,accuracy_C#,recall_A,quality_A], [accuracy_B,recall_B,quality_B], [accuracy_C,recall_C,quality_C]
...
def analyze_feedback_distribution(df,task):
    ...
    # Use Counter to count occurrences of feedback types for both True and False cases
        ...
        feedback_false_rates = {k: v / total_false for k, v in feedback_false_counts.items()} if total_false > 0 else {}
        feedback_true_rates_sorted = dict(sorted(feedback_true_rates.items(), key=lambda item: item[1], reverse=True))
        feedback_false_rates_sorted = dict(sorted(feedback_false_rates.items(), key=lambda item: item[1], reverse=True))
    else:
...
def calculate_followup_accuracy(df,task):
    ...
    #import pdb;pdb.set_trace()
    try:
        followup_accuracy_main_true = [(float(max(eval(i))) >= 1) if  eval(i) != [] else 0 for i in main_true_df['correctness_followup']].count(True) /len(main_true_df)
    except:
        followup_accuracy_main_true = 0
    try:
        followup_accuracy_main_false = [(float(max(eval(i))) >= 1) if  eval(i) != [] else 0 for i in main_false_df['correctness_followup']].count(True)/len(main_false_df)
    except:
...
if __name__ == "__main__":
    ...
    head = ['model',"acc_E1","acc_E2","acc_E3","followup_accuracy_true","followup_accuracy_false","feedback_true","feedback_false","error_type"]
    ...
    for file in files:
        ...
        if task == 'math':
            ...
            write_csv_row([model,accuracy_A,accuracy_B,accuracy_C,followup_accuracy_true,followup_accuracy_false,feedback_true,feedback_false,error_type],output_path)
        else:
            write_csv_row([model]+accuracy_A+accuracy_B+accuracy_C+[feedback_true,feedback_false],output_path)
        ...
        for id,dfs in enumerate([level1,level2,level3,level4,level5]):
            ...
            if task == 'math':
                ...
                write_csv_row([id,accuracy_A,accuracy_B,accuracy_C,followup_accuracy_true,followup_accuracy_false,feedback_true,feedback_false,error_type],output_path)
            else:
                write_csv_row([id]+accuracy_A+accuracy_B+accuracy_C+[feedback_true,feedback_false],output_path)
