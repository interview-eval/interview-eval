...
class HFMATHLoader:
    ...
    def load_samples(self, N):
        ...
        if remaining_samples_needed > 0:
            ...
            remaining_samples_needed = min(
                remaining_samples_needed, len(remaining_pool)
            )

            if remaining_samples_needed > 0:
                # Randomly or deterministically select the remaining samples
                remaining_samples = remaining_pool.sample(
                    remaining_samples_needed, random_state=1
                )
...
if __name__ == "__main__":
    ...
    loader_test_intermediate_algebra = HFMATHLoader(
        split="test", category="intermediate_algebra"
    )
    ...
    loader_train_intermediate_algebra = HFMATHLoader(
        split="train", category="intermediate_algebra"
    )
