...
def parse_arguments():
    ...
    parser.add_argument("--state_threshold_followup", type=int, default=1)
    ...
    parser.add_argument("--followup_flag", type=int, default=1)    
...
def seed_question(N):
    ...
    uniform_samples_1 = df_1.groupby('level').apply(lambda x: x.sample(n=min(len(x), n_samples_per_level), random_state=21)).reset_index(drop=True)
    uniform_samples_2 = df_2.groupby('level').apply(lambda x: x.sample(n=min(len(x), n_samples_per_level), random_state=21)).reset_index(drop=True)
...
def main():
    ...
    seed_questions = pd.DataFrame([{'id':0,'initial_question':'question','level':1,'solution':'solution'},{'id':1,'initial_question':'question','level':1,'solution':'solution'}])
    ...
    # Moderator Setting
    moderator_model = ChatModel.create_model(args.model_moderator)
    moderator = Moderator(
        model= ChatModel.create_model(args.model_moderator),
        summarizer= ChatModel.create_model(args.model_moderator),
        system_message=SystemMessage(content=""),
        queries=seed_questions,
        references=None,
        store_session=True,
        agents_name=names,
        followup_flag = args.followup_flag,
        start_query_index=start_query_index,
        state_threshold=args.state_threshold,
        state_threshold_followup=args.state_threshold_followup,
